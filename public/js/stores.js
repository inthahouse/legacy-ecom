...
$(function () {
  ...
  function getStoredView() {
    try {
      return localStorage.getItem('tno_store_view') === 'table' ? 'table' : 'list';
    } catch (e) {
  ...
  var LAT_MIN = 24, LAT_MAX = 57, LNG_MIN = -128, LNG_MAX = -50;
  ...
  function currentFilter() {
    ...
    var list = stores.filter(function (s) {
      ...
      if (q) {
        var hay = (s.name + ' ' + s.city + ' ' + s.regionName + ' ' + s.postal + ' ' + s.address).toLowerCase();
    ...
    if (userLoc) {
      list.forEach(function (s) { s._dist = haversineKm(userLoc, s); });
      list.sort(function (a, b) { return a._dist - b._dist; });
    } else {
      list.forEach(function (s) { delete s._dist; });
      list.sort(function (a, b) {
        return a.country < b.country ? -1 : a.country > b.country ? 1 :
  ...
  function renderList(list, $out) {
    $.each(list, function (i, s) {
      var $item = $(
        '<div class="store-item' + (s.id === selectedId ? ' is-selected' : '') + '" data-id="' + s.id + '">' +
        (s.isFlagship ? '<div class="store-flag">Flagship</div>' : '') +
        '<h3>' + s.name + '</h3>' +
        '<p>' + s.address + ', ' + s.city + ', ' + s.region + ' ' + s.postal + '</p>' +
        '<p>' + s.phone + ' · Mon–Fri ' + s.hours['Mon-Fri'] + '</p>' +
        (s._dist !== undefined ? '<p class="store-dist">' + s._dist.toFixed(0) + ' km away</p>' : '') +
        '<div class="store-links"><a href="/stores/' + s.id + '">Store details</a>' +
  ...
  function renderTable(list, $out) {
    ...
    $.each(list, function (i, s) {
      var $row = $(
        '<tr class="store-row' + (s.id === selectedId ? ' is-selected' : '') + '" data-id="' + s.id + '">' +
        '<td>' + (s.isFlagship ? '<span class="store-flag">Flagship</span>' : '') + s.name + '</td>' +
        '<td>' + s.address + ', ' + s.city + ', ' + s.region + ' ' + s.postal + '</td>' +
        '<td>' + s.phone + '</td>' +
        '<td>' + s.hours['Mon-Fri'] + '</td>' +
        (showDist ? '<td class="store-dist">' + s._dist.toFixed(0) + ' km</td>' : '') +
        '<td><div class="store-links"><a href="/stores/' + s.id + '">Details</a>' +
  ...
  function render() {
    ...
    $('#stores-result-count').text(list.length + ' store' + (list.length === 1 ? '' : 's') + ' found');

    if (!list.length) {
      $out.append('<div class="empty-state"><p>No stores match. Try widening your search.</p></div>');
    ...
    $.each(list, function (i, s) {
      ...
      var $pin = $('<div class="map-pin' + (s.id === selectedId ? ' is-selected' : '') + '" data-id="' + s.id + '" title="' + s.name + '"></div>')
  ...
  function select(id, scrollList) {
    ...
    $('.store-item[data-id=' + id + '], .store-row[data-id=' + id + ']').addClass('is-selected');
    $('.map-pin[data-id=' + id + ']').addClass('is-selected');

    var s = null;
    $.each(stores, function (i, st) { if (st.id === id) s = st; });
    if (s) {
      $('#map-selected').addClass('is-visible').html(
        '<strong>' + s.name + '</strong><br>' +
        s.address + ', ' + s.city + ', ' + s.region + '<br>' +
        s.phone + ' &middot; <a href="/stores/' + s.id + '">Details &rarr;</a>'
      );
    }
    if (scrollList) {
      var $item = $('.store-item[data-id=' + id + '], .store-row[data-id=' + id + ']');
      if ($item.length) {
        $('.stores-list').animate({ scrollTop: $('.stores-list').scrollTop() + $item.position().top - 10 }, 250);
      }
    }
  }
  ...
  $('#store-country').on('change', function () {
    ...
    if (c && $('#store-region option:selected').data('country') && $('#store-region option:selected').data('country') !== c) {
      $('#store-region').val('');
    }
    render();
  });
